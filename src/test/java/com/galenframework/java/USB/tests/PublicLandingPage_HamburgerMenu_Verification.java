...
public class PublicLandingPage_HamburgerMenu_Verification extends GalenTestBase {
    ...
    *********  TEST SUITE RELATED TO USB SITE **********
    ...
    public void usbPublicLandingPage(TestDevice device) throws IOException {
        ...
        if(device.getName().equalsIgnoreCase("Mobile"))
        getDriver().findElement(By.xpath(".//button[contains(@class,'burger-menu')]")).click();
        checkLayout("/specs/Public_Landing_Page_hambugermenu.spec", device.getTags());
    }
